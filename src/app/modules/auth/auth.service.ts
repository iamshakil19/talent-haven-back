...
const forgotPass = async (payload: { email: string }) => {
  ...
  const passResetToken = await jwtHelpers.createResetToken({ _id: user._id }, config.jwt.secret as string, '5m')

  const resetLink: string = config.resetLink + `email=${user?.email}&token=${passResetToken}`
